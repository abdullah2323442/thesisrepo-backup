...
// PDF file URL
const pdfUrl = '{{ route('supervisor.reports.submissions.view', [$report, $submission]) }}';
...
document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('{{ route('supervisor.reports.submissions.annotations.store', [$report, $submission]) }}', {
            method: 'POST',
            headers: {
...
async function confirmSendFeedback() {
    ...
    try {
        const response = await fetch(`/supervisor/reports/{{ $report->id }}/submissions/{{ $submission->id }}/annotations/${currentSessionId}/send-feedback`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route('supervisor.reports.show', $report) }}';
            }, 2000);
        } else {
