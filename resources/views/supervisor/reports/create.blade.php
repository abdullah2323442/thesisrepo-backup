...
@section('page-description', 'Create a new report for your group')

@section('content')
<div class="container mx-auto max-w-4xl">
    <div class="bg-white rounded-lg shadow-md">
        <div class="p-6">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Create New Report</h2>
            ...
            <form action="{{ route('supervisor.reports.store') }}" method="POST" x-data="{ reportType: 'general' }">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select name="type" id="type" required x-model="reportType"
                        ...
                        <option value="general" {{ old('type') == 'general' ? 'selected' : '' }}>General Report</option>
                        <option value="final" {{ old('type') == 'final' ? 'selected' : '' }}>Final Report</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea name="supervisor_message" id="supervisor_message" rows="4"
                              ...
                              placeholder="Enter instructions or message for students about what they need to do for this report...">{{ old('supervisor_message') }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="extra_input" id="extra_input" rows="4"
                              ...
                              placeholder="Any additional notes or instructions for students">{{ old('extra_input') }}</textarea>
                    @error('extra_input')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
