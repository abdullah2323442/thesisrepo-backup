...
class ReportAnnotationController extends Controller
{
    ...
    public function annotate(Report $report, StudentReportSubmission $submission)
    {
        ...
        if (!$supervisor) {
            return redirect()->route('panel-member.dashboard')
                ->with('error', 'Panel member profile not found.');
        }

        // Verify the report belongs to a group where this user is a panel member
        ...
        return view('panel-member.reports.annotate', compact('report', 'submission', 'annotationSessions'));
    }
    ...
    public function store(Request $request, Report $report, StudentReportSubmission $submission)
    {
        ...
        if (!$supervisor) {
            return response()->json(['error' => 'Panel member profile not found.'], 403);
        }
        ...
        try {
            ...
            $annotationSession = ReportAnnotationSession::create([
                ...
                'created_by_type' => 'panel_member', // Track that this was created by panel member
            ]);

            DB::commit();

            Log::info('Panel member annotation session saved as draft', [
            ...
            Log::error('Failed to create panel member annotation session', [
                'error' => $e->getMessage(),
                'report_id' => $report->id,
                'submission_id' => $submission->id,
                'panel_member_id' => $supervisor->id,
    ...
    public function sendFeedback(Request $request, Report $report, StudentReportSubmission $submission, ReportAnnotationSession $annotationSession)
    {
        ...
        try {
            ...
            Log::info('Panel member finding students for annotation notification', [
                'annotation_session_id' => $annotationSession->id,
                'report_id' => $report->id,
            ...
            foreach ($groupStudents as $groupStudent) {
                ...
                if ($user) {
                    ...
                    Log::warning('No user found for student in panel member annotation notification', [
            ...
            if ($students->isNotEmpty()) {
                ...
                foreach ($students as $student) {
                    try {
                        ...
                        Log::info('Panel member annotation notification sent to student', [
                            'user_id' => $student->id,
                            'annotation_session_id' => $annotationSession->id,
                            'panel_member_id' => $supervisor->id
                        ]);
                    } catch (\Exception $e) {
                        Log::error('Failed to send panel member annotation notification to student', [
                            'user_id' => $student->id,
                            'error' => $e->getMessage(),
                            'panel_member_id' => $supervisor->id
                        ]);
            ...
            Log::info('Panel member annotation feedback sent successfully', [
                'annotation_session_id' => $annotationSession->id,
            ...
            Log::error('Failed to send panel member annotation feedback', [
                'error' => $e->getMessage(),
                'annotation_session_id' => $annotationSession->id,
                'panel_member_id' => $supervisor->id
            ]);
    ...
    public function history(Report $report, StudentReportSubmission $submission)
    {
        ...
        return view('panel-member.reports.annotation-history', compact('report', 'submission', 'annotationSessions'));
    }
}