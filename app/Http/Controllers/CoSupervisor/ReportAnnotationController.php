...
class ReportAnnotationController extends Controller
{
    ...
    public function annotate(Report $report, StudentReportSubmission $submission)
    {
        ...
        if (!$supervisor) {
            return redirect()->route('co-supervisor.dashboard')
                ->with('error', 'Co-supervisor profile not found.');
        }

        // Verify the report belongs to a group co-supervised by this supervisor
        if ($report->group->co_supervisor_id !== $supervisor->id) {
        ...
        return view('co-supervisor.reports.annotate', compact('report', 'submission', 'annotationSessions'));
    }
    ...
    public function store(Request $request, Report $report, StudentReportSubmission $submission)
    {
        ...
        if (!$supervisor) {
            return response()->json(['error' => 'Co-supervisor profile not found.'], 403);
        }
        ...
        try {
            ...
            $annotationSession = ReportAnnotationSession::create([
                ...
                'created_by_type' => 'co_supervisor', // Track that this was created by co-supervisor
            ]);

            DB::commit();

            Log::info('Co-supervisor annotation session saved as draft', [
            ...
            Log::error('Failed to create co-supervisor annotation session', [
                'error' => $e->getMessage(),
                'report_id' => $report->id,
                'submission_id' => $submission->id,
                'co_supervisor_id' => $supervisor->id,
    ...
    public function sendFeedback(Request $request, Report $report, StudentReportSubmission $submission, ReportAnnotationSession $annotationSession)
    {
        ...
        // Verify the annotation session belongs to this user
        ...
        try {
            ...
            Log::info('Co-supervisor finding students for annotation notification', [
                'annotation_session_id' => $annotationSession->id,
                'report_id' => $report->id,
            ...
            foreach ($groupStudents as $groupStudent) {
                ...
                if ($user) {
                    ...
                    Log::warning('No user found for student in co-supervisor annotation notification', [
            ...
            if ($students->isNotEmpty()) {
                ...
                foreach ($students as $student) {
                    try {
                        ...
                        Log::info('Co-supervisor annotation notification sent to student', [
                            'user_id' => $student->id,
                            'annotation_session_id' => $annotationSession->id,
                            'co_supervisor_id' => $supervisor->id
                        ]);
                    } catch (\Exception $e) {
                        Log::error('Failed to send co-supervisor annotation notification to student', [
                            'user_id' => $student->id,
                            'error' => $e->getMessage(),
                            'co_supervisor_id' => $supervisor->id
                        ]);
            ...
            Log::info('Co-supervisor annotation feedback sent successfully', [
                'annotation_session_id' => $annotationSession->id,
            ...
            Log::error('Failed to send co-supervisor annotation feedback', [
                'error' => $e->getMessage(),
                'annotation_session_id' => $annotationSession->id,
                'co_supervisor_id' => $supervisor->id
            ]);
    ...
    public function history(Report $report, StudentReportSubmission $submission)
    {
        ...
        return view('co-supervisor.reports.annotation-history', compact('report', 'submission', 'annotationSessions'));
    }
}